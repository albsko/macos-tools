...
class StageUtil {
    nonisolated(unsafe) private static let windowManagerDefaults = UserDefaults(
        suiteName: "com.apple.WindowManager")
    nonisolated(unsafe) private static let dockDefaults = UserDefaults(suiteName: "com.apple.dock")
    ...
    static func isStageStripVisible(_ screen: NSScreen? = .main) -> Bool {
        ...
        let infos = WindowUtil.getWindowList().filter { info in
            ...
            let screens = NSScreen.screens.filter {
                $0.frame.minY <= frame.minY && frame.maxY <= $0.frame.maxY
            }
            var infoScreen: NSScreen?
            if stageStripPosition == .left {
                infoScreen = screens.min {
                    abs(frame.minX - $0.frame.minX) < abs(frame.minX - $1.frame.minX)
                }
            } else {
                infoScreen = screens.min {
                    abs($0.frame.maxX - frame.maxX) < abs($1.frame.maxX - frame.maxX)
                }
            }
    ...
    static func getStageStripWindowGroup(_ windowId: CGWindowID, _ screen: NSScreen? = .main)
        -> [CGWindowID]?
    {
