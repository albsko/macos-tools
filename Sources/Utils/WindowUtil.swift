...
public class WindowUtil {
        ...
        -> [WindowInfo]
    {
        ...
        if let ids {
            ...
            rawInfos = CGWindowListCopyWindowInfo(
                [all ? .optionAll : .optionOnScreenOnly, .excludeDesktopElements], kCGNullWindowID)
        }
        if let rawInfos {
            ...
            for i in 0..<count {
                ...
                let info = WindowInfo(
                    id: id, level: level, frame: frame, pid: pid, processName: processName)
                infos.append(info)
            }
        }
