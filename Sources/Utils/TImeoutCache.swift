...
class TimeoutCache<Key: Hashable, Value> {
    ...
    private var purgeRestrictionExpired: Bool {
        DispatchTime.now().uptimeMilliseconds > purgeRestrictionExpirationTimestamp
    }
    ...
    subscript(key: Key) -> Value? {
        ...
        set {
            ...
            let entry = Entry(
                key: key, value: value, expirationTimestamp: expirationTimestamp, previous: tail)
    ...
    private func purge() {
        ...
        purgeRestrictionExpirationTimestamp =
            DispatchTime.now().uptimeMilliseconds + (100 * timeout)
    }
